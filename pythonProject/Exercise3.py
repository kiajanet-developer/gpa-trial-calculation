...
for number in student:
    multiplied_marks.append(number + (5 / 100 * number))
print(multiplied_marks)


counter = 0
for mark in multiplied_marks:
    if 80 <= float(mark) <= 100:
        grade = 'A'
        print(f"For {course_units[counter]}, grade is {grade}")
        grade_point = 5.0
        total_grade_point += (credit_units[counter] * grade_point)
...
print('total_grade_point:', total_point)


gpa = total_grade_point / total_credit_units
print(f' hello {name}, your gpa  after 5% increase in your target is {gpa}')
